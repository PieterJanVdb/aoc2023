...
let extract_digit line =
  line
  |> Re.matches Re.(compile (repn digit 1 (Some 1)))
  |> fun t -> List.hd_exn t ^ List.last_exn t |> Int.of_string
;;

let solve input =
  input |> String.split_lines |> List.map ~f:extract_digit |> List.fold ~init:0 ~f:( + )
;;
